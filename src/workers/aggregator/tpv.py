...
class TPVAggregator(ProcessWorker): 
    def __init__(self) -> None:
        ...
        self.pending_state = PendingStateStore(worker_label="tpv_aggregator")
        ...
        self.metadata_eof_state = MetadataEOFStateStore(
            required_metadata_types={'stores'},
    ...
    def process_batch(self, batch: list[Dict[str, Any]], client_id: ClientId):
        ...
        # If metadata just became ready, replay pending batches before handling this one
    ...
    def _store_pending_batch(
        ...
        if stored:
            logger.info(
                "[TPV-AGGREGATOR] Metadata not ready for client %s, stored batch as pending "
                "(state=%s)",
                client_id,
                metadata_state,
            )
            ...
            logger.info(
                "[TPV-AGGREGATOR] Metadata not ready for client %s, pending batch already stored "
                "(state=%s)",
                client_id,
                metadata_state,
            )
    ...
    def _process_pending_batches(self, client_id: ClientId) -> None:
        ...
        try:
            ...
            logger.info(
                "[TPV-AGGREGATOR] Replaying %s pending batches for client %s after metadata ready",
                len(pending_entries),
                client_id,
            )
            ...
            for entry in pending_entries:
                ...
                if pending_uuid and self.processed_messages.has_processed(client_id, pending_uuid):
                    logger.info(
                        "[TPV-AGGREGATOR] Skipping already processed pending batch %s for client %s",
                        pending_uuid,
                        client_id,
                    )
                    continue
    ...
    def _on_metadata_ready(self, client_id: ClientId) -> None:
        threading.Thread(
            ...
            args=(client_id,),
            daemon=True,
            name=f"TPVPendingReplayer-{client_id}",
        ).start()
